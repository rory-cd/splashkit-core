    ...
    const fs::path &outputFilepath)
{
    ...
    {
        writeTestCase(test, file, category, AST.globals, AST.functions);
    }
    ...
    if (auto *expr = dynamic_cast<const Expression*>(assertion.expression.get()))
    {
        ...
        if (auto *bin = dynamic_cast<const BinaryExpression*>(expr))
        {
            ...
            if (bin->op == "==")
            {
                ...
                // If there's a literal on the right, swap places
                if (auto *literal = dynamic_cast<const LiteralExpression*>((bin->right).get()))
                {
...
std::string CSharpTranslator::translateExpression(const Expression &expr)
{
    ...
    throw std::runtime_error("Unsupported Expression type in Translator: translateExpression()");
}
...
{
    std::string result = toPascalCase(expr.functionName);
    result += "(";
...
{
    file << indt() << funcDecl.returnType << " " << toPascalCase(funcDecl.name) << "(";
